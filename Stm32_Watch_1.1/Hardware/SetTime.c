...
extern uint8_t  KeyNum;		//定义在Menu.c的KeyNum
...
void show_SetTime_TimeUIMenu(void)			//设置Time时间
{
	MTimer Ti;
	MyRTC_ReadTime(&Ti);
	OLED_Printf(0,0,OLED_8X16,"时%2d",Ti.hour);
	OLED_Printf(0,16,OLED_8X16,"分%2d",Ti.minute);
	OLED_Printf(0,32,OLED_8X16,"秒%2d",Ti.second);
}
