...
void Show_Clock_UIMenu(void)
{
	...
	else if(mood_choice==2)
	{
		OLED_ShowImage(107,0,16,16,Unhappy);
	}
	else if(mood_choice==3)
	{
...
int Slip_Menu(void)
{
	while(1)
	{
		...
		if(slip_menu_setflag_temp==1)		//退出
		{
			return 0;
		}
		else if(slip_menu_setflag_temp==2)		//计时器
		{
			Slip_StopWatch();
		}
		else if(slip_menu_setflag_temp==3)		//首页心情
		{
			Slip_SetMood();
		}
		else if(slip_menu_setflag_temp==4)		//游戏
		{
			Slip_PlayGame();
		}
		else if(slip_menu_setflag_temp==5)		//水平仪
		{
			Slip_PlayGradienter();
		}
		else if(slip_menu_setflag_temp==6)		//灯
		{
			Slip_Light();
		}
