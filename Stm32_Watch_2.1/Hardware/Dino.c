...
void Show_Barrier(void)
{
	if(barrier_pos>=143)	//等上一个障碍物完全移出屏幕外才取随机数
		barrier_choice=rand()%4;
	if(barrier_choice<3)		//小型的障碍
	{
	...
	{
		OLED_ShowImage(127-barrier_pos,32,16,30,Big_Barrier[barrier_choice-3]);
		barrier_p.minX=127-barrier_pos;
		barrier_p.maxX=barrier_p.minX+16;
		barrier_p.minY=32;
		barrier_p.maxY=62;
...
#define LOW 1
#define HIGH 2
void Show_Dino(uint8_t High_Low)
{
	if(dino_jump_flag)	
	{
		if(High_Low==LOW)
			jump_pos=28*sin((float)(PI*jump_t/1000));		//跳跃高度通过正弦函数算28sin(wt)
		else if(High_Low==HIGH)
			jump_pos=45*sin((float)(PI*jump_t/1000));		//跳跃高度通过正弦函数算45sin(wt)
	}
	if(dino_jump_flag==0)
	{
		if(cloud_pos%2)		OLED_ShowImage(0,44,16,18,Dino[1]);
		else	OLED_ShowImage(0,45,16,18,Dino[0]);
	}
	else
	{
		OLED_ShowImage(0,45-jump_pos,16,18,Dino[2]);
	}
	
	dino_p.minX=0;
	dino_p.maxX=16;
	dino_p.minY=45-jump_pos;
...
void DinoGame_Animation(void)
{
	barrier_pos=0;
	score=0;
	uint8_t high_jump_flag=1;
	while(1)
	{
		if(Key_Check(0,KEY_SINGLE))	//跳跃
		{
			high_jump_flag=1;
			dino_jump_flag=1;		//按Key1就是跳跃
		}
		else if(Key_Check(0,KEY_DOUBLE))	//跳跃
		{
			high_jump_flag=2;
			dino_jump_flag=1;		//按Key1就是跳跃
		}
		else if(Key_Check(2,KEY_SINGLE))	//按Key3退出游戏
		{
			OLED_Clear();
			OLED_ShowString(28,24,"Game Over",OLED_8X16);
			OLED_Update();
