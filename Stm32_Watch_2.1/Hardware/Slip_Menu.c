...
void Slip_StopWatch(void)
{
	while(1)
	{
		...
		if(watch_setflag_temp==1)		//确认退出
		{
			return;
		}
		else if(watch_setflag_temp==2)		//确认开始
		{
			start_timing_flag=1;
		}
		else if(watch_setflag_temp==3)		//确认停止
		{
			start_timing_flag=0;
		}
		else if(watch_setflag_temp==4)		//确认清零
		{
			start_timing_flag=0;
			watch_hour=watch_min=watch_sec=0;
		}
		
		switch(watch_flag)			//显示界面
		{
			case 1:
...
void Slip_SetMood(void)
{
	while(1)
	{
		...
		if(mood_setflag_temp==1)		//确认选开心
		{
			Set_MoodChoice(MOOD_HAPPY);
			return;
		}
		else if(mood_setflag_temp==2)		//确认选不开心
		{
			Set_MoodChoice(2);
			return;
		}
		else if(mood_setflag_temp==3)			//确认选焦虑
		{
			Set_MoodChoice(MOOD_ANXIETY);
			return;
		}
		
		switch(mood_flag)		//显示界面
		{
			case 1:
				OLED_Clear();
...
void Slip_PlayGame(void)
{
	while(1)
	{
		...
		if(game_flag_temp==1)		//确认退出
		{
			return;
		}
		else if(game_flag_temp==2)		//确认游玩
		{
			DinoGame_Animation();
		}
		
		switch(game_flag)		//显示界面
		{
			case 1:
				Slip_ShowGmaeUI();
				OLED_ReverseArea(0,0,16,16);
...
void Slip_PlayGradienter(void)
{
	while(1)
	{
		if(Key_Check(2,KEY_SINGLE))	//单机按下了，确认
		{
			OLED_Clear();
//			OLED_Update();		//这里注释掉看有没有影响
			return;
		}
...
void Slip_Light(void)
{
	...
	while(1)
	{
		...
		if(light_flag_temp==1)		//确认退出
		{
			return;
		}
		else if(light_flag_temp==2)		//确认转换常亮开关
		{
			GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
			GPIO_Init(GPIOA, &GPIO_InitStructure);
			LED1_Turn();
		}
		else if(light_flag_temp==3)		//确认转换呼吸灯开关
		{
			GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
			GPIO_Init(GPIOA, &GPIO_InitStructure);
			if(light_mode==1)
				light_mode=2;
			else if(light_mode==2)		//确认转换闪烁开关
			{
				light_mode=1;
				PWN_SetCompare1(0);
				GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
